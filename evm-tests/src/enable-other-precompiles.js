// Script to enable all Bittensor precompiles using admin_utils
...
async function main() {
  try {
    ...
    for (const [name, id] of Object.entries(PrecompileEnum)) {
      console.log(`Enabling ${name} precompile (ID: ${id})...`);
      try {
        ...
        console.log(`${name} precompile is now enabled!`);
      } catch (error) {
        console.error(`Error enabling ${name} precompile:`, error.message);
      }
    }
